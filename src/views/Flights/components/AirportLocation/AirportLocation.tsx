...
const AirportLocation = (props: Props): JSX.Element => {
  ...
  const [options, setOptions] = useState<readonly MockLocation[]>([]);
  const [airportOptions, setAirPortOptions] = useState<readonly AirptLocatn[]>(
    [],
  );
  ...
  const handleProductChange = (
    event: SelectChangeEvent<typeof mockLocation>,
    value,
  ) => {
        ...
        <TextField
          ...
          InputProps={{
            ...
            startAdornment: (
              <React.Fragment>
                {' '}
                {props.takeoff ? <FlightTakeoffIcon /> : <FlightLandIcon />}
              </React.Fragment>
            ),
            endAdornment: (
              <React.Fragment>
